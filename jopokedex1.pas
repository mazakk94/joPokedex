...
begin
  wsk := first;
  if wsk=nil then
    showmessage('Lista nie posiada elementów!')
  else
    begin
      while wsk<>nil do
        begin
...
if eLP.text='' then
  showmessage('wprowadŸ dane!(LP)')
else if eNazwa.Text='' then
  showmessage('wprowadŸ dane!(nazwa)')
else if gatunekbox.itemindex=0 then
  showmessage('wprowadŸ dane!(gatunek)')
else
  begin
    sprawdzLP;
...
begin
  ...
  Reset(wejscie);
//first := nil;  //trzeba bêdzie jeszcze zwolniæ pamiêæ po wszystkich elementach
nastepny := nil;
...
else showmessage('Zaznacz jakiœ element!!!')
end;
